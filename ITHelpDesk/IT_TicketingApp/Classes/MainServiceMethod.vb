...
Public Class MainServiceMethod
    ...
    Dim objBSS As New clsDBAccess(My.Settings.BSSAppDB, DBEngineType.SQL)
    ...
    Public Function GetUserDetailsbyUserID(ByVal UserID As Integer) As DataTable
        Try
            Dim Para As String(,) = {{"@UserID", UserID}}
            Dim dt = objIT.SP_Datatable("sp_GetUserDetailsbyUserIDUpdate", Para)
            Return dt
        Catch ex As Exception
            Throw ex
        End Try
    End Function
    Public Function GetMainControl(ByVal roleID As Integer) As ArrayList
        Dim dt As New DataTable
        ...
        Try
            ...
            Dim para As String(,) = {{"@RoleID", roleID}}
            dt = objBSS.SP_Datatable("GetMainControl", para)
            dt.TableName = "GetMainControl"


            If dt.Rows.Count > 0 Then
                For i As Integer = 0 To dt.Rows.Count - 1
                    ar.Add(dt.Rows(i)("RoleName") & ":" & dt.Rows(i)("Formname") & ":" & dt.Rows(i)("Modulename"))
                Next
            Else
                ar.Add("No Record")
            End If
    ...
    End Function
    Public Function AuthenticateUser(ByVal username As String, ByVal Password As String) As DataTable
        Dim dt As New DataTable
        Try

            Dim para As String(,) = {{"@username", username}, {"@Password", Password}}
            dt = objBSS.SP_Datatable("VerifyUser", para)
            dt.TableName = "VerifyUser"
            Return dt
